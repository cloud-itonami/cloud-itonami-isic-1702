(ns corrugated.phase
  "Phase 0->3 staged rollout for the corrugated-board/box-manufacturing
  ...
  consequence (a corrugator/converting-line maintenance window means
  production downtime, and the equipment ends up actually touched); it
  is always a human plant supervisor's call. `corrugated.governor`'s
  `equipment-control-blocked-violations` HARD-blocks direct-actuation
  ...
  has only ONE member (`:log-production-batch`) -- no separate
...
(defn verdict->disposition
  "Map a Corrugated Packaging Plant Operations Governor verdict to a
