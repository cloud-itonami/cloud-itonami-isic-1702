(ns corrugated.operation
  "CorrugatedPackagingOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (CorrugatedPackagingAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Corrugated Packaging Plant Operations Governor (:govern) and the
...
(defn build
  "Compiles a CorrugatedPackagingOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `corrugated.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; CorrugatedPackagingAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Corrugated Packaging Plant Operations Governor -- independent censor.
      (g/add-node :govern
