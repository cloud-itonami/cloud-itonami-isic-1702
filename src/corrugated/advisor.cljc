(ns corrugated.advisor
  "CorrugatedPackagingAdvisor -- the *contained intelligence node* for
  the corrugated-board/box-manufacturing plant-operations coordination
  actor.

  It normalizes production-batch patches (board/container-grade/
  quantity/burst-strength/edge-crush-test), drafts a corrugator/
  printer-slotter/flexo-folder-gluer/die-cutter maintenance scheduling
  proposal against a piece of equipment, drafts a safety-concern flag,
  and drafts an outbound corrugated-board-sheet/box shipment
  ...
  a real corrugator/converting-line-equipment actuation or freight
  dispatch. Every output is censored downstream by `corrugated.governor`
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct corrugator/
                                 ; converting-line-equipment-control
                                 ; effect
  ...
  contract every caller of this actor agrees to) -- `corrugated.governor`
  ...
  (:require #?(:clj  [clojure.edn :as edn]
            ...
            [corrugated.registry :as registry]
...
(defn- log-production-batch
  ...
  burst-strength/edge-crush readings or verification status. High
...
(defn- schedule-maintenance
  "Draft a corrugator/converting-line maintenance-window scheduling
  ...
  `corrugated.governor` NEVER trusts this report -- it independently
...
(defn- flag-safety-concern
  ...
  `corrugated.phase`: no phase ever adds this op to a phase's `:auto`
  set; `corrugated.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound corrugated-board-sheet/box shipment coordination
  proposal against a production batch. The advisor passes through the
  caller's own claimed quantity -- it does NOT invent one, and
  `corrugated.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは段ボール・製函工場プラント運用コーディネーターの助言者です。"
       ...
       "コルゲータ・製函コンバーティングライン設備の直接操作を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `corrugated.governor`
