(ns corrugated.store
  "SSoT for the corrugated-board/box-manufacturing plant-operations
  ...
  for dev/tests/demo, no deps. Mirroring `cloud-itonami-isic-1701`'s
  own pulp/paper vertical (docs/adr/0001-architecture.md Decision 1),
  this vertical is self-contained (no external corrugated capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             corrugated-board/container-grade/quantity/
                             quality (burst-strength/edge-crush) record.
                             ...
                             truth.
    - `equipment`         -- a corrugator/converting-line (printer-
                             slotter, flexo-folder-gluer, die-cutter)
    ...
    - `maintenance`       -- a scheduled corrugator/converting-line
                             maintenance-window DRAFT against a piece
                             of equipment (`corrugated.registry`'s
    ...
    - `shipments`         -- a proposed outbound corrugated-board-
                             sheet/box shipment DRAFT
                             (`corrugated.registry`'s
                             `register-shipment`).
  ...
  audit trail a plant owner or box buyer trusting this coordinator
  needs."
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (corrugated.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (corrugated.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :grade :single-wall-c-flute :customer "buyer-box-north"
                :quantity-units 5000.0 :burst-strength-kpa 700.0 :edge-crush-kn-m 6.5
                :verified? true :registered? true
                :shipped-quantity-units 1000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :grade :regular-slotted-container :customer "buyer-box-east"
                :quantity-units 800.0 :burst-strength-kpa 900.0 :edge-crush-kn-m 8.0
                :verified? true :registered? true
                :shipped-quantity-units 750.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :grade :double-wall-board :customer "buyer-box-south"
                :quantity-units 6000.0 :burst-strength-kpa 450.0 :edge-crush-kn-m 4.5
                :verified? false :registered? false
                :shipped-quantity-units 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"equip-001" {:id "equip-001" :kind :corrugator
                :verified? true :registered? true
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `corrugated.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `corrugated.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  shipment coordinated against it); one verified+registered corrugator
  unit (schedulable for maintenance), one UNVERIFIED/unregistered
  die-cutter unit (blocks any maintenance scheduling against it) -- so
