(ns corrugated.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `corrugated.store` ns docstring for why a second (Datomic-backed)
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:grade :single-wall-c-flute}})
    (is (= :single-wall-c-flute (:grade (store/batch s "batch-001"))))
